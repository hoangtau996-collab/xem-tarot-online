...
export const VisitorCounter = ({ lang = 'vi', variant = 'footer' }) => {
  ...
  if (variant === 'navbar') {
    return (
      <div
        className="hidden 2xl:flex items-center gap-2 px-2.5 py-1 rounded-full bg-purple-950/40 border border-amber-400/30 text-xs text-amber-200/90 shadow-sm backdrop-blur-md hover:border-amber-400/60 transition-all cursor-default"
        title={`${label}: ${value}`}
      >
